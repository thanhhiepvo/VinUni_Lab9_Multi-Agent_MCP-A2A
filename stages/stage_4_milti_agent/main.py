...
def check_routing(state: LegalState) -> list[Send]:
    """Keyword-based routing: dispatch parallel specialist tasks via Send API."""
    ...
    if not tasks:
        print("  [Node: check_routing] No specialist keywords matched -> aggregate")
        return [Send("aggregate", state)]

    routed = [t.node for t in tasks]
    print(f"  [Node: check_routing] Dispatching to: {routed}")
    return tasks
...
QUESTION = (
    "A tech company shared user data without consent (GDPR violation) and avoided "
    "paying taxes on overseas revenue. What are the legal, tax, and privacy consequences?"
)
